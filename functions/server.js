...
const {
  authenticateAsync,
} = require("@themidastouch/parcelperfect/src/authentication");
const fetchDestination = require("./src/fetchDestination");
const createQuote = require("./src/createQuote");
const {
  updateService,
  quoteToCollection,
} = require("@themidastouch/parcelperfect/src/quotes");
...
app.post("/createOrder", async (request, response) => {
  ...
  if (wcorder.status === "processing") {
    ...
    try {
      ...
      const token = await authenticateAsync(username, password).catch(
        (error) => {
      ...
      const destination = await fetchDestination(wcorder, token).catch(
        (error) => {
          logger.error("fetchDestination", error);
        }
      );
      ...
      if (destination) {
        const quote = await createQuote(wcorder, destination, token).catch(
          (error) => {
        ...
        const service = await updateService(quote.quoteno, "ONX", token).catch(
          (error) => {
        ...
        const collection = await quoteToCollection(quote.quoteno, token).catch(
          (error) => {
            logger.error("quoteToCollection", error);
          }
        );
